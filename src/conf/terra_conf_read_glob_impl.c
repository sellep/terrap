...
BOOL terra_conf_read_glob_impl(terra_conf * const conf, FILE * const f)
{
	...
	while ((read = getline(&line, &buf_len, f)) != -1)
	{
		...
		if (strncmp(line, CONF_TRAN_PIN, sizeof(CONF_TRAN_PIN) - 1) == 0 && line[sizeof(CONF_TRAN_PIN) - 1)] == '=')
		{
			conf->tran_pin = atoi(line + sizeof(CONF_TRAN_PIN));
		}
		else if (strncmp(line, CONF_TRAN_REP, sizeof(CONF_TRAN_REP) - 1) == 0 && line[sizeof(CONF_TRAN_REP) - 1)] == '=')
		{
			conf->tran_rep = atoi(line + sizeof(CONF_TRAN_REP));
		}
		else if (strncmp(line, CONF_SOCK_CHAN, sizeof(CONF_SOCK_CHAN) - 1) == 0 && line[sizeof(CONF_SOCK_CHAN) - 1)] == '=')
		{
			conf->sock_chan = atoi(line + sizeof(CONF_SOCK_CHAN));
		}
		else if (strncmp(line, CONF_SOCK_CODE_A_ON, sizeof(CONF_SOCK_CODE_A_ON) - 1) == 0 && line[sizeof(CONF_SOCK_CODE_A_ON) - 1)] == '=')
		{
			conf->sock_code_a_on = atoi(line + sizeof(CONF_SOCK_CODE_A_ON));
		}
		else if (strncmp(line, CONF_SOCK_CODE_A_OFF, sizeof(CONF_SOCK_CODE_A_OFF) - 1) == 0 && line[sizeof(CONF_SOCK_CODE_A_OFF) - 1)] == '=')
		{
			conf->sock_code_a_off = atoi(line + sizeof(CONF_SOCK_CODE_A_OFF));
		}
		else if (strncmp(line, CONF_SOCK_CODE_B_ON, sizeof(CONF_SOCK_CODE_B_ON) - 1) == 0 && line[sizeof(CONF_SOCK_CODE_B_ON) - 1)] == '=')
		{
			conf->sock_code_b_on = atoi(line + sizeof(CONF_SOCK_CODE_B_ON));
		}
		else if (strncmp(line, CONF_SOCK_CODE_B_OFF, sizeof(CONF_SOCK_CODE_B_OFF) - 1) == 0 && line[sizeof(CONF_SOCK_CODE_B_OFF) - 1)] == '=')
		{
			conf->sock_code_b_off = atoi(line + sizeof(CONF_SOCK_CODE_B_OFF));
		}
		else if (strncmp(line, CONF_SOCK_CODE_C_ON, sizeof(CONF_SOCK_CODE_C_ON) - 1) == 0 && line[sizeof(CONF_SOCK_CODE_C_ON) - 1)] == '=')
		{
			conf->sock_code_c_on = atoi(line + sizeof(CONF_SOCK_CODE_C_ON));
		}
		else if (strncmp(line, CONF_SOCK_CODE_C_OFF, sizeof(CONF_SOCK_CODE_C_OFF) - 1) == 0 && line[sizeof(CONF_SOCK_CODE_C_OFF) - 1)] == '=')
		{
