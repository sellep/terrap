...
BOOL terra_conf_read_sched_temps(terra_conf * const conf, FILE * const f)
{
	...
	while ((read = getline(&line, &buf_len, f)) != -1)
	{
		...
		if(!terra_conf_read_sched(&temp->sched, f)) HANDLE_ERROR("[terra_conf_read_sched_temps] failed to parse temp schedule\n");

		//schedule temp act_lim

		if ((read = getline(&line, &buf_len, f)) == -1) HANDLE_ERROR("[terra_conf_read_sched_temps]unexpected end of temp schedule section\n");
		...
		else HANDLE_ERROR("[terra_conf_read_sched_temps] temp schedule act_lim expected\n");
		...
		else HANDLE_ERROR("[terra_conf_read_sched_temps] temp schedule deact_lim expected\n");
		...
		else HANDLE_ERROR("[terra_conf_read_sched_temps] temp schedule invert expected\n");

		//schedule temp end

		if ((read = getline(&line, &buf_len, f)) == -1) HANDLE_ERROR("[terra_conf_read_sched_temps] unexpected end of temp schedule section\n");

		if (strncmp(line, CONF_SCHED_TEMP_SECTION_END, sizeof(CONF_SCHED_TEMP_SECTION_END) - 1) != 0) HANDLE_ERROR("[terra_conf_read_sched_temps] invalid section end\n");
